import boto3
import botocore # Going to use exception handling
import click
...
@click.option('--all', 'list_all', default=False, is_flag=True,
    help='List all snapshots for each volume, not just the most recent') # It will only list all if '--all' is present
...
def create_snapshots(project):
    ...
    for i in instances:
        ...
        i.stop()
        i.wait_until_stopped() # Wait until the instances stopped
        for v in i.volumes.all():
        ...
        i.start() # We dont need to wait for the snapshot to complete
